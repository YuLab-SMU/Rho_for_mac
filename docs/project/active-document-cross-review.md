...
| `release/active-0.4.0-dev.33-candidate-checklist.md` | active replacement source contract; deterministic source repairs, AGPL LIC-1/LIC-2, SP-READY1, and PR #46 are integrated; EDITOR-VIEWPORT-R1 local implementation, affected validation, review, and NEWS reconciliation pass; one exact fork-main review-only Windows rehearsal is authorized while hosted upstream integration and external Windows signing readiness remain open | sole `0.4.0-dev.33` source identity and any future exact candidate, installed, MAC5, publication, and updater evidence | the authorized `Rho_for_mac` rehearsal may produce only expiring Actions artifacts and no Release; no candidate before EDITOR-VIEWPORT-R1 protected upstream integration, SignPath approval, real production configuration, two-stage signing implementation/verification, and all named release gates; installed acceptance, MAC5, publication, and candidate updater mutation remain separate |
